...
- 右：グローバルナビ 14px / `letter-spacing: 0.06em`。カテゴリ3つ（GTM／業務の仕組み化・AI活用／起業・事業づくり）→ 縦罫線1px → 記事一覧・運営元について（`--muted`）。
...
3. サービス：bizFlag／bizLab／bizShip の名前・1行説明・各サービスサイトへのリンクのみ（詳細は語らない）
...
- 固有名（bizFlag、bizLab、bizShip、GTM）はそのまま英字で表記。
- 執筆者の表記は「執筆・監修者」で統一。
